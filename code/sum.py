...
def time_to_seconds(time_str):
    try:
        ...
        if match:
            ...
            raise ValueError(f"Time string '{time_str}' is not in the format HH:MM:SS.")
    except Exception as e:
        print(f"Error parsing time string: {e}")
        return 0

def find_summary_regions(srt_filename, start_time, end_time, language="english"):
    enc = chardet.detect(open(srt_filename, "rb").read())['encoding']
    srt_file = pysrt.open(srt_filename, encoding=enc)
    ...
    for item in srt_file:
        item_start = item.start.hours * 3600 + item.start.minutes * 60 + item.start.seconds
        item_end = item.end.hours * 3600 + item.end.minutes * 60 + item.end.seconds
        if item_start >= start_seconds and item_end <= end_seconds:
            summary.append((item_start, item_end))
    return summary
...
def create_summary(filename, regions, target_duration):
    ...
    # Adjust the duration of the summary to be 1/4th of the target duration
    summary = summary.set_duration(target_duration)
    
    return summary

def get_summary(filename="1.mp4", subtitles="1.srt", start_time=None, end_time=None):
    if start_time is None or end_time is None:
        print("Start and end times must be specified.")
        return False

    if not re.match(r'\d+:\d+:\d+', start_time) or not re.match(r'\d+:\d+:\d+', end_time):
        print("Start and end times must be in the format HH:MM:SS.")
        return False

    start_seconds = time_to_seconds(start_time)
    end_seconds = time_to_seconds(end_time)
    segment_duration = end_seconds - start_seconds
    target_duration = segment_duration / 2 # Set target duration to 1/4th of the segment duration

    regions = find_summary_regions(subtitles, start_time, end_time, "english")
    summary = create_summary(filename, regions, target_duration)
    base, ext = os.path.splitext(filename)
    output = "{0}_summarised.mp4".format(base)
    summary.to_videofile(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', '--keep-original-file', help="Keep original movie & subtitle file",
                        action="store_true", default=False)
    parser.add_argument('-st', '--start-time', help="Start time of the summary (e.g., 4:25)")
    parser.add_argument('-et', '--end-time', help="End time of the summary (e.g., 12:55)")
    ...
    if not url:
        get_summary(args.video_file, args.subtitles_file, start_time, end_time)
    else:
        movie_filename = download_video(url)
        subtitle_filename = download_cc_as_srt(url)
        summary_retrieval_process = multiprocessing.Process(target=get_summary,
                                                            args=(movie_filename, subtitle_filename, start_time, end_time))
        summary_retrieval_process.start()
        summary_retrieval_process.join()
        if not keep_original_file:
            os.remove(movie_filename)
